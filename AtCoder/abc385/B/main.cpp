...
int main() {
    ...
    auto getNext = [&](ll r, ll c, char t)  {
        ...
        auto [y ,x] = cur;
        if (y < 0 || y >= H || x < 0 || x >= W) return P{r, c};
        if (S[y][x] == '#') return P{r, c};
        if (S[y][x] == '@'){
            used.insert(cur);
        }
        return cur;
    ...
    P cur = {X - 1, Y - 1};
    repr(t, T) {
        cur = getNext(cur.first, cur.second, t);
    }
    cout << cur.first+1 << " " << cur.second+1 << " " << used.size() << endl;
    return 0;
}
