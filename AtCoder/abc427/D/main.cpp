...
int main() {
    ...
    while (T--) {
        ...
        map<P, string> mp;
        auto dfs = [&](auto dfs, ll u, ll p, ll k) -> string {
            P key = {u, p, k};
            if (mp.count(key)) return mp[key];
            if (k == 0) {
                return mp[key] = (S[u] == 'A') ? A : B;
            }
            for (auto&& v : to[u]) {
                if (names[p] == dfs(dfs, v, p^1, k - 1)) {
                    return mp[key] = names[p];
                }
            }
            return  mp[key] = names[p^1];
        };
        string ans = dfs(dfs, 0, 0, K * 2);
        cout << ans << '\n';
    }
    return 0;
}
