...
int main() {
    ...
    vector DP(q+1, vector(q+1, linf));
    DP[0][0] = 0;
    rep(i, N) {
        vector pre(q+1, vector(q+1, linf));
        swap(DP, pre);
        rep(j,q+1){
            rep(k,q+1){
                if(ll nj = j+B[i]; nj <= q) chmin(DP[nj][k], pre[j][k] + (A[i] != 0 ? 1:0));
                if(ll nk = k+B[i]; nk <= q) chmin(DP[j][nk], pre[j][k] + (A[i] != 1 ? 1:0));
                chmin(DP[j][k], pre[j][k] + (A[i] != 2 ? 1:0));
            }
        }
    }
    ll ans = DP[q][q] == linf ? -1 : DP[q][q];
    cout << ans << endl;
    return 0;
}
