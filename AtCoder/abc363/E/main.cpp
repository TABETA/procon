...
int main() {
    ...
    CIN(ll, Y);
    vvll A(H, vll(W));
    vector alive(H, vector(W, true));
    vector below(H, vector(W, false));
    auto is_coast = [&](P p) {
        auto [r, c] = p;
        vector<P> ret;
        for (auto &&q : {
    ...
    auto judge = [&](P p) {
        ...
        while (!Q.empty()) {
            ...
            alive[y][x] = false;
            --ans;
            for (auto pp : next_adjacents(q, H, W)) {
                auto [y, x] = pp;
                if (!alive[y][x]) continue;
                if (!is_coast(pp)) continue;
                if (!below[y][x]) continue;
                Q.push(pp);
            }
        }
    };
