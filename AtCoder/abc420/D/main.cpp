...
int main() {
    ...
    auto next_adjacents = [&](T p) {
        ...
        })
        {
            ...
            if (A[b][y][x] == 'x' || A[b][y][x] == '#') continue;
            ll nb = (A[b][y][x] == '?') ? 1 - b : b;
            ret.emplace_back(nb, y, x);
        }
        return ret;
    };
    queue<T> Q;
    Q.emplace(s);
    vector cost(2, vvll(H, vll(W, linf)));
    cost[0][get<1>(s)][get<2>(s)] = 0;
    while(!Q.empty()){
        ...
        for(auto&& nk: next_adjacents(k)){
            auto [nb, ny, nx] = nk;
            if(cost[nb][ny][nx] < linf)continue;
            cost[nb][ny][nx] = cost[b][y][x] + 1;
            Q.emplace(nk);
        }
    }
    cout << -1 << endl;
