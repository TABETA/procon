...
int main() {
    ...
    while (T--) {
        ...
        using T = pair<ll, vector<P>>;
        priority_queue<T, vector<T>, greater<T>> Q;
        Q.emplace(0, G);
        map<vector<P>, ll> cost;
        while (!Q.empty()) {
            auto [c, u] = Q.top();
            Q.pop();
            if (cost.count(u) && cost[u] < c) {
                continue;  // 既により小さいコストで到達済み
            }
            if (u.size() <= 1) {
                cout << c << endl;
                break;
            ...
            ll m = u.size() - 1;
            {
                ...
                ll i = m-1;
                {
                    ...
                    auto& v = V[i];
                    auto w = V[i + 1];
                    ll nc = c + w.first - v.second - 1;
                    v.second = w.second;
                    V.pop_back();
                    if (!cost.count(V) || cost[V] > nc) {
                        cost[V] = nc;
                        Q.emplace(nc, V);
                    }
                }
