...
template <> inline bool chmax<string>(string& a, const string& b) {bool compare = a.size() < b.size(); if (compare) a = b; return compare;}
...
int main() {
    ...
    rep(i,N){
        rep(j,M){
            ...
            chmax(len, dp[i+1][j+1]);
        }
    }
    string ans;
    repd(i,N){
        repd(j,M){
            if(dp[i+1][j+1] == len && s[i] == t[j]){
                ans += s[i];
                len--;
                break;
            }
