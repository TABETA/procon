...
auto solve(long long N, long long Q, std::vector<long long> x, std::vector<long long> y, std::vector<long long> q) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, Q, std::move(x), std::move(y), std::move(q)) << endl;
    return 0;
}
