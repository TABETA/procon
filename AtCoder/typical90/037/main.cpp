...
auto solve(long long W, long long N, std::vector<long long> L, std::vector<long long> R, std::vector<long long> V) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(W, N, std::move(L), std::move(R), std::move(V)) << endl;
    return 0;
}
