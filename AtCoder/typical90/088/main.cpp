...
auto solve(long long N, long long Q, std::vector<long long> A, std::vector<long long> X, std::vector<long long> Y) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, Q, std::move(A), std::move(X), std::move(Y)) << endl;
    return 0;
}
