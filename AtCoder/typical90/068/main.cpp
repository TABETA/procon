...
auto solve(long long N, long long Q, std::vector<long long> T, std::vector<long long> X, std::vector<long long> Y, std::vector<long long> V) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, Q, std::move(T), std::move(X), std::move(Y), std::move(V)) << endl;
    return 0;
}
