...
auto solve(long long N, long long T, std::vector<long long> AX, std::vector<long long> AY, std::vector<long long> BX, std::vector<long long> BY) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, T, std::move(AX), std::move(AY), std::move(BX), std::move(BY)) << endl;
    return 0;
}
