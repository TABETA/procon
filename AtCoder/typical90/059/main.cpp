...
auto solve(long long N, long long M, long long Q, std::vector<long long> X, std::vector<long long> Y, std::vector<long long> A, std::vector<long long> B) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, M, Q, std::move(X), std::move(Y), std::move(A), std::move(B)) << endl;
    return 0;
}
