...
auto solve(long long N, long long Q, std::vector<long long> A, std::vector<long long> L, std::vector<long long> R, std::vector<long long> V) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, Q, std::move(A), std::move(L), std::move(R), std::move(V)) << endl;
    return 0;
}
