...
auto solve(long long N, long long Q, std::vector<long long> x, std::vector<long long> y, std::vector<long long> z, std::vector<long long> w) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, Q, std::move(x), std::move(y), std::move(z), std::move(w)) << endl;
    return 0;
}
