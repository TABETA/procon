...
auto solve(long long N, long long M, std::vector<long long> C, std::vector<long long> L, std::vector<long long> R) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, M, std::move(C), std::move(L), std::move(R)) << endl;
    return 0;
}
