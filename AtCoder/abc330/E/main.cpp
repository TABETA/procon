...
auto solve(long long N, long long Q, std::vector<long long> A, std::vector<long long> i, std::vector<long long> x) {
    ll ans = inf;
...
int main() {
    ...
    cout << solve(N, Q, std::move(A), std::move(i), std::move(x)) << endl;
    return 0;
}
