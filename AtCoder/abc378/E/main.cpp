...
int main() {
    ...
    reps(r, 1, N + 1) {
        ...
        now += B[r] * r;
        now -= C[r] - C[0];
        now += fw.sum(B[r]+1, M)*M;
        // l:1->rの区間の合計をしている。
        // 元々求めたいのはmod値なので、この区間においてB[l-1] > B[r]となる値が出てきたらMを加算しなければいけない
        // これはB[r]+1からM-1までの範囲において、B[l-1]が存在する個数を数えられれば解決できる
        // これは過去に出てきた値をfwに記録しておけば実現できる
        fw.add(B[r], 1);
        ans += now;
    }
    cout << ans << endl;
