...
int main() {
    ...
    ll rem = K%N;
    rep(_,rem){
        auto binary_search = [&](ll ac, ll wa) -> ll {
            while (abs(ac - wa) > 1) {
                auto f = [&](ll wj){
                    ll a = B.sum(ac,wj);
                    ll b = B.sum(wj,wa);
        ...
        ll i = binary_search(0, N+1);
        B.add(i,1);
    }
    ll X = 0;
    auto dfs = [&](auto dfs, vll u) -> ll{
        auto p = minmax_element(all(u));
        chmax(X, *p.second - *p.first);
        if(u.size() == 1) return X;
        vll v(u.size()/2);
        rep(i,u.size()/2){
            v[i] = u[i*2] + u[i*2+1];
        }
