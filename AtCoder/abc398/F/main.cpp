...
int main() {
    ...
    string ans = "";
    auto f = [&](bool duplicated){
        for(auto i = mp.rbegin(), j = pm.rbegin(); i != mp.rend() && j != pm.rend(); ++i, ++j){
            if(*i != *j) return;
        }
        string now = "";
        rep(j,mp.size()){
        ...
        auto won = now;
        if(duplicated) won.pop_back();
        reverse(all(won));
        now += won;
        if(ans == "" || ans.size() > now.size()){
            ans = now;
        }
    };
    --i;++j;
