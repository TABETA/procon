...
int main() {
    ...
    rep(_,T){
        ...
        auto dfs = [&](auto dfs, ll u) -> bool {
            ...
            if(ng.count(u)) return memo[u] = false;
            if(ans == u) return memo[u] = true; // すでに探索済み
            bool now = false;
            rep(i,N){
                ll mask = 1LL<<i;
                if(u & mask) continue; // i番目のビットが立っているならスキップ
                ll next = u | mask; // i番目のビットを立てる
                now |= dfs(dfs, next); // 再帰的に探索
                if(now) break;
            }
            return memo[u] = now;
        };
